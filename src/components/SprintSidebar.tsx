...
import { Calendar, ChevronLeft, ChevronRight, ListIcon, Plus } from "lucide-react";
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-4">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="font-medium">Sprints Semanais</span>
          </div>
          
          <div className="space-y-2 mt-3">
            {sprints.map(sprint => (
              <div key={sprint.id} className="flex items-center py-2">
                <ListIcon className="h-4 w-4 mr-2 text-gray-500" />
                <div>
                  <div className="font-medium">{sprint.name}</div>
                  <div className="text-sm text-gray-500">{formatSprintDates(sprint)}</div>
                </div>
              </div>
            ))}
          </div>
