...
const addPdfStatus = (pdf: jsPDF, isArchived: boolean, yPosition: number) => {
  pdf.setFontSize(12);
  pdf.text(
    `Status: ${isArchived ? 'Arquivada' : 'Ativa'}`,
    20, 
    yPosition
  );
  return yPosition + 10;
...
export const exportSprintToPDF = async (sprint: Sprint) => {
  ...
  yPosition = addPdfStatus(pdf, sprint.archived || false, yPosition);
